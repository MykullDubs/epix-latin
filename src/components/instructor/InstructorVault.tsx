    ...
    const { displayFiles, sortedFolders, allAbsoluteFolders } = useMemo(() => {
        ...
        if (isSearching) {
            ...
            baseFiles.forEach((a: any) => {
                ...
                // 🔥 FIXED: Explicitly typed 'p' as string to satisfy the strict TS compiler
        ...
        <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 overflow-hidden select-none animate-in fade-in duration-500 relative transition-colors duration-300">
            ...
            {pendingDelete && (
                <div className="absolute bottom-28 left-1/2 -translate-x-1/2 z-[9999] bg-slate-900 dark:bg-slate-800 text-white px-6 py-4 rounded-[2rem] shadow-[0_40px_80px_rgba(0,0,0,0.6)] border-4 border-rose-500 flex flex-col sm:flex-row items-center gap-6 animate-in slide-in-from-bottom-12 zoom-in-95 duration-300">
                    <div className="text-center sm:text-left">
                        ...
                        <p className="text-xs text-slate-300 font-bold max-w-[250px] truncate">{pendingDelete.length === 1 ? `"${pendingDelete[0].title}" will be permanently destroyed.` : `${pendingDelete.length} artifacts will be permanently destroyed.`}</p>
                    </div>
                    <div className="flex gap-2 shrink-0 w-full sm:w-auto">
                        <button onClick={() => setPendingDelete(null)} className="flex-1 sm:flex-none px-6 py-3 bg-slate-800 hover:bg-slate-700 dark:bg-slate-950 dark:hover:bg-slate-900 rounded-xl text-xs font-black uppercase tracking-widest transition-colors">Cancel</button>
                        <button onClick={handleConfirmDelete} className="flex-1 sm:flex-none px-6 py-3 bg-rose-600 hover:bg-rose-500 rounded-xl text-xs font-black uppercase tracking-widest transition-colors shadow-lg shadow-rose-600/20">Destroy</button>
                    </div>
                </div>
            )}
            ...
            <header className="h-24 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-6 md:px-10 flex justify-between items-center shrink-0 z-50 shadow-sm transition-colors duration-300">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tighter uppercase leading-none">Global Vault</h2>
                        <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] mt-1">Manage & Deploy Artifacts</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <div className="relative w-full sm:w-56 hidden md:block">
                        ...
                        <input 
                            ...
                            className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white font-bold rounded-xl pl-9 pr-4 py-2 outline-none focus:border-indigo-500 transition-all shadow-inner text-xs"
                        />
                    </div>
                    <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl shrink-0 shadow-inner border border-slate-200 dark:border-slate-700">
                        <button onClick={() => setSortBy('recent')} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${sortBy === 'recent' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}>
                            <Clock size={12} /> Recent
                        </button>
                        <button onClick={() => setSortBy('alpha')} className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${sortBy === 'alpha' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-4 md:px-8 py-8 pb-32">
                ...
                {!searchQuery && (
                    <div className="flex flex-wrap items-center gap-2 mb-6 px-2 text-sm font-black uppercase tracking-widest text-slate-500 shrink-0">
                        <button onClick={() => setCurrentPath([])} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-colors ${currentPath.length === 0 ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-400' : 'hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                        ...
                        {currentPath.map((segment, idx) => {
                            ...
                            return (
                                <React.Fragment key={idx}>
                                    ...
                                    <button onClick={() => setCurrentPath(currentPath.slice(0, idx + 1))} className={`px-3 py-1.5 rounded-lg transition-colors ${isLast ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-400' : 'hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-300'}`}>
                                        {segment}
                                    </button>
                                </React.Fragment>
                            );
                ...
                {!searchQuery && sortedFolders.length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
                        {sortedFolders.map((folder: string) => (
                            <button key={folder} onClick={() => setCurrentPath([...currentPath, folder])} className="p-5 bg-indigo-50 dark:bg-indigo-500/10 border-2 border-indigo-100 dark:border-indigo-500/20 rounded-[1.5rem] flex items-center gap-4 hover:border-indigo-400 dark:hover:border-indigo-400 transition-all active:scale-95 group text-left shadow-sm">
                ...
                {displayFiles.length === 0 && sortedFolders.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-max">
                        {displayFiles.map((artifact: any) => {
                            ...
                            return (
                                <div key={`${artifact.artifactType}_${artifact.id}`} className={`border-2 rounded-[2rem] p-6 flex flex-col transition-all group relative overflow-hidden min-h-[220px] ${isSelected ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-400 dark:border-indigo-500 shadow-[0_10px_30px_rgba(99,102,241,0.2)] scale-[0.98]' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-600 hover:shadow-lg'}`}>
                                    <button onClick={(e) => { e.stopPropagation(); toggleSelection(artifact.id, artifact.artifactType, artifact.displayTitle); }} className="absolute top-6 right-6 z-20 text-slate-300 hover:text-indigo-500 transition-colors">
                                    ...
                                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100 dark:border-slate-800 relative z-10 shrink-0">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1"><Clock size={10} /> {new Date(artifact.timestamp || Date.now()).toLocaleDateString()}</span>
                                        ...
                                        <div className={`flex gap-2 transition-opacity ${selectedCount > 0 ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
                                            <button onClick={() => setPendingDelete([{ id: artifact.id, type: artifact.artifactType, title: artifact.displayTitle }])} className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/20 flex items-center justify-center transition-colors"><Trash2 size={16} strokeWidth={3} /></button>
                                            <button onClick={() => onEditArtifact(artifact.id, artifact.artifactType)} className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-500/20 flex items-center justify-center transition-colors"><Edit3 size={16} strokeWidth={3} /></button>
                                            <button onClick={() => onLaunchLive(artifact.id, artifact.artifactType)} className="w-10 h-10 rounded-full bg-indigo-600 text-white hover:bg-indigo-500 flex items-center justify-center transition-transform active:scale-90 shadow-md"><Play size={16} fill="currentColor" className="ml-0.5" /></button>
                                        </div>
                                    </div>
                                </div>
                            );
            ...
            {selectedCount > 0 && (
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[8000] bg-slate-900 dark:bg-slate-800 text-white px-3 py-3 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.4)] border border-slate-700 flex items-center gap-4 animate-in slide-in-from-bottom-12 duration-300 w-[95%] md:w-auto max-w-3xl">
                    <div className="bg-indigo-500 text-white w-10 h-10 rounded-full flex items-center justify-center font-black shrink-0">{selectedCount}</div>
                    ...
                    <div className="flex gap-2 flex-1 justify-end relative">
                        <div className="relative">
                            <button onClick={() => setShowFolderMenu(!showFolderMenu)} className={`px-4 py-2.5 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest flex items-center gap-2 transition-colors border ${showFolderMenu ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-800 border-slate-700 hover:bg-slate-700 text-slate-300'}`}><FolderPlus size={16} /> Move</button>
                            {showFolderMenu && (
                                <div className="absolute bottom-full left-0 mb-4 w-72 bg-slate-800 border border-slate-700 rounded-2xl p-4 shadow-2xl flex flex-col gap-3 z-50">
                                    ...
                                    <div className="flex gap-2">
                                        <input type="text" placeholder="e.g. Math/Algebra" value={newFolderName} onChange={e => setNewFolderName(e.target.value)} className="flex-1 bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs font-mono text-emerald-400 outline-none focus:border-indigo-500" />
                                        <button onClick={() => newFolderName.trim() && handleBulkMoveToFolder(newFolderName)} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-xl text-white font-black text-xs transition-colors">Move</button>
                                    </div>
                                    {allAbsoluteFolders.length > 0 && (
                                        ...
                                        <div className="max-h-40 overflow-y-auto custom-scrollbar flex flex-col gap-1">
                                            <button onClick={() => handleBulkMoveToFolder('')} className="w-full text-left px-3 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest text-rose-400 hover:bg-rose-500/10 transition-colors">[Move to Root]</button>
                                            {allAbsoluteFolders.map((f: string) => (
                                                <button key={String(f)} onClick={() => handleBulkMoveToFolder(String(f))} className="w-full flex items-center gap-2 text-left px-3 py-2 rounded-lg text-xs font-mono font-bold text-slate-300 hover:bg-slate-700 hover:text-white transition-colors truncate"><Folder size={12} className="shrink-0 text-slate-500" /><span className="truncate">{String(f)}</span></button>
                                            ))}
                                        </div></>
                                    )}
                                </div>
                        ...
                        <button onClick={() => { const items = Object.entries(selectedArtifacts).map(([id, data]) => ({ id, type: data.type, title: data.title })); setPendingDelete(items); setShowFolderMenu(false); }} className="px-4 py-2.5 bg-slate-800 hover:bg-rose-500/20 border border-slate-700 hover:border-rose-500/50 text-slate-300 hover:text-rose-400 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest flex items-center gap-2 transition-colors"><Trash2 size={16} /> Delete</button>
                        <button onClick={clearSelection} className="px-3 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-400 hover:text-white rounded-xl flex items-center justify-center transition-colors shrink-0 ml-2"><X size={16} /></button>
                    </div>
                </div>
            )}
        </div>
