...
const FORGE_PARTS: Record<string, string[]> = {
    ...
    top: ['antenna', 'antennaCrooked', 'bulb01', 'floating', 'glowingBulb01', 'glowingBulb02', 'horns', 'lights', 'pyramid', 'radar', 'none'],
    sides: ['antenna01', 'antenna02', 'cables01', 'cables02', 'round', 'square', 'squareAssymetric', 'none'],
    texture: ['camo01', 'camo02', 'circuits', 'dirty01', 'dirty02', 'dots', 'none']
};
...
export default function AvatarForge({ currentConfig, onSave, onClose }: any) {
    // Merge new categories into their existing config so it doesn't break older saves
    ...
    // 🔥 DYNAMIC URL BUILDER (Safely handles 'none' selections)
    const buildUrl = () => {
    ...
    return (
        <div className="fixed inset-0 z-[7000] flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2.5rem] shadow-2xl border-4 border-slate-100 dark:border-slate-800 relative z-10 animate-in zoom-in-95 duration-300 overflow-hidden flex flex-col">
                ...
                {/* HEADER & LIVE PREVIEW */}
                <div className="bg-slate-100 dark:bg-slate-800 p-8 pb-6 relative flex flex-col items-center border-b border-slate-200 dark:border-slate-700">
                            ...
                            {FORGE_PARTS[activeTab].map((part) => {
                                ...
                                return (
                                    <button 
                                        ...
                                        className={`py-4 px-2 rounded-xl text-xs font-bold capitalize transition-all active:scale-95 flex items-center justify-center text-center ${isSelected ? 'bg-indigo-600 text-white shadow-md shadow-indigo-500/30 border-2 border-indigo-500' : 'bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-indigo-300 dark:hover:border-indigo-500/50'}`}
                                    >
