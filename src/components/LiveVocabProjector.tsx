...
export default function LiveVocabProjector({ deck, classId, activeClass, onExit }: any) {
    ...
    // Fallback ID to prevent Firebase crashes if the deck object varies
    const safeDeckId = deck?.id || deck?.key || 'custom_vocab_run';

    // DYNAMIC QUIZ GENERATOR (With robust ID fallbacks)
    ...
    }, [classId, safeDeckId, quizQuestions]); // Added quizQuestions to dependency array to ensure it fires

    // AUTO-SKIP ENGINE: Jump to 0 if everyone locked in
    useEffect(() => {
        if (liveState?.quizState === 'active' && isAutoPilot) {
            ...
            if (totalStudents > 0 && currentAnswers >= totalStudents) {
                setTimeLeft(0); // Immediately trigger the interval to clear
            }
        }
    ...
    if (!isAutoPilot && (!liveState?.quizState || liveState?.quizState === 'waiting')) {
        return (
            <div className="h-full bg-slate-950 text-white flex flex-col items-center justify-center p-12 relative overflow-hidden">
                ...
                <div className="z-10 text-center max-w-4xl w-full mt-12">
                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-6">
                            {activeClass?.students?.map((s: any, i: number) => (
                                <div key={i} className="flex flex-col items-center gap-2 animate-in zoom-in duration-500" style={{ animationDelay: `${i * 50}ms` }}>
                                    <div className="w-14 h-14 rounded-2xl bg-slate-800 border-2 border-slate-700 flex items-center justify-center text-xl font-black text-slate-500 shadow-inner">
                                        {(s.name?.[0] || s.email[0]).toUpperCase()}
                                    </div>
                                    <span className="text-[10px] font-black uppercase text-slate-600 truncate w-16 text-center">{s.name || s.email.split('@')[0]}</span>
                                </div>
                            ))}
                            {(!activeClass?.students || activeClass.students.length === 0) && (
                                <div className="col-span-full py-8 text-slate-600 font-black uppercase tracking-widest text-sm">
                            ...
                            <div className="pt-8 border-t border-slate-800">
                                ...
                                <div className="flex justify-center gap-4 flex-wrap">
                                    {Object.entries(answers).map(([email, ansId]: any, i) => {
                                        ...
                                        // Attempt to match the email back to a student name if available
                                    ...
                                    {answerCount === 0 && (
                                        <p className="text-slate-600 font-bold italic">No responses received this round.</p>
                                    )}
                                </div>
                            </div>
                        </div>
