          ...
          <div className="animate-in slide-in-from-bottom-4 duration-500 delay-50">
            <div className="flex justify-between items-end mb-3 ml-1">
              <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">
                <Brain size={16} className="text-indigo-500"/> Diagnostics
              </h3>
              <span className="text-[10px] font-bold bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-md">CEFR A1–C1</span>
            </div>
            <div 
              onClick={() => onStartExam('preposition')}
              className="group relative bg-slate-900 hover:bg-slate-800 border-2 border-slate-800 hover:border-indigo-500/80 rounded-[2rem] p-6 cursor-pointer transition-all duration-300 flex flex-col justify-between overflow-hidden shadow-xl active:scale-[0.98] text-left"
            >
              <div className="absolute -right-10 -bottom-10 w-40 h-40 bg-indigo-500/15 rounded-full blur-2xl group-hover:bg-indigo-500/25 transition-all pointer-events-none" />
              <div className="flex justify-between items-start mb-4">
                <div className="w-12 h-12 bg-indigo-500/20 border border-indigo-500/30 rounded-2xl flex items-center justify-center text-indigo-400 group-hover:scale-110 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300 shadow-inner">
                  <Compass size={24} strokeWidth={2.5} />
                </div>
                <span className="inline-block px-2.5 py-1 bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 rounded-full text-[10px] font-black uppercase tracking-widest">
                  Adaptive Test
                </span>
              </div>
              <div>
                <h4 className="text-xl font-black text-white mb-1 group-hover:text-indigo-300 transition-colors">
                  Preposition Exam
                </h4>
                <p className="text-xs text-slate-400 font-medium leading-relaxed mb-4">
                  Evaluate your mastery across time, location, and direction. Automatically adjusts from A1 to C1.
                </p>
              </div>
              <div className="pt-4 border-t border-slate-800 flex items-center justify-between text-xs font-black uppercase tracking-widest text-indigo-400 group-hover:text-indigo-300">
                <span>Begin Placement</span>
                <div className="w-7 h-7 rounded-full bg-slate-800 flex items-center justify-center group-hover:translate-x-1 transition-transform text-white">
                  <ArrowRight size={14} />
                </div>
              </div>
            </div>
  ...
  if (activeExam === 'preposition') {
      return (
          <div className="relative min-h-screen w-full bg-slate-950">
              ...
              >
                  <ArrowLeft size={14} strokeWidth={2.5} /> Exit
              </button>
              <PrepositionExam />
          </div>
      );
  ...
  if (!user) {
      ...
      return (
          <LandingPage 
              ...
              onStartPlacement={() => setActiveExam('placement')}
              onStartPrepositions={() => setActiveExam('preposition')} 
          />
      );
  }
  ...
  return (
    <div className="bg-slate-50 min-h-screen w-full font-sans text-slate-900 flex justify-center items-start relative overflow-hidden">
      <div className="w-full max-w-md h-[100dvh] bg-white shadow-2xl relative overflow-hidden flex flex-col">
        ...
        {/* BOTTOM NAVIGATION (4 BUTTONS - HIDDEN IN FULLSCREEN EXAMS) */}
        {!isFullscreenMode && <StudentNavBar activeTab={activeTab} setActiveTab={setActiveTab} />}
      </div>
      <style>{` 
